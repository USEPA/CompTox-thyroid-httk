...
ggsave(plot = uniq.positives,
       ...
       device = "tiff",
       filename = "./figures/300dpi/uniq_positives_v2.tiff")
...
ggsave(plot = uniq.positives,
       ...
       width = 190, height = 152,
       device = "png",
       filename = "./figures/uniq_positives_v2.png")
...
venn_allcolors <- display_venn(class = "Colors", hitc.class.dat = new.merged.colors, class_var = "main_class")

# after selectivity filter 
venn_selcolors <- display_venn(class = "Colors", hitc.class.dat = new.merged.colors.sel, class_var = "main_class")

plot_grid(venn_allcolors, venn_selcolors, ncol = 2, 
          labels = c("All Colors", "Selective Only"))
...
# combine the plots into one figure
iyd.a <- ggarrange(plotlist = sig.iyd.colors, 
...
# combine the plots into one figure
iyd.b <- ggarrange(plotlist = sig.iyd.colors2, 
          labels = names(sig.iyd.colors2),
          font.label = list(size = 10),
          nrow = 1, ncol = 2)

annotate_figure(iyd.a, top = text_grob("All Red/Blues", 
               color = "red", face = "bold", size = 14))

annotate_figure(iyd.b, top = text_grob("Selective Red/Blues Only", 
               color = "red", face = "bold", size = 14))
```
...
# combine the plots into one figure
regA <- ggarrange(plotlist = sig.iyd.classes, 
...
# combine the plots into one figure
regB <- ggarrange(plotlist = sig.iyd.classes2, 
          labels = names(sig.iyd.classes2),
          font.label = list(size = 10),
          nrow = 1, ncol = 2)

annotate_figure(regA, top = text_grob("All Registered Substances", 
               color = "red", face = "bold", size = 14))

annotate_figure(regB, top = text_grob("Selective Registered Substances Only", 
               color = "red", face = "bold", size = 14))
```
...
## Figure 4 and 5 of Truong et al. 
```{r ggplts-4-and-5}
...
venn_obj <- ggvenn(testedChems.by.assay, 
       ...
       text_size = 2.5,
       set_name_size = 2.5) +
  scale_y_continuous(expand = expansion(mult = .1)) + # fix labels cropped at the top with this ggplot trick; this adds more padding to the y-axis
  scale_x_continuous(expand = expansion(mult = .1)) # fix "DIO1" label being slightly cropped in the same way
...
barchart <- ggplot(class.hitc.dist, 
  ...
  theme(legend.position = "none",
        ...
        strip.text = element_text(size = 8),  # controls facet label size 
        axis.text.y = element_text(size = 6),
        axis.text.x = element_text(size = 7)) +
       # plot.title.position = "plot", 
       # plot.title = element_text(size = 8, hjust = 0.1, vjust = 0.5)) +
  coord_flip() +
  facet_grid(hitcall ~ assay, scales = "free_y") +
  labs(x = "Class", 
       y = "% of Chemicals with the chain:alkaneLinear_C(8,10,12,14) chemotype found Active or Inactive for Target")
       # title = "Class Distribution of Chemicals with chain:alkaneLinear_C(8,10,12,14) chemotypes") # remove title for more space

# this is the only way I found for modifying individual facets and their themes
# keep active row theme the same 
# for(i in 1:4) {
#   barchart <- barchart +
#     theme(axis.text.y = element_text(size = 7))
# }
# 
# # make inactive row's y axis tick labels a tad smaller
# for(i in 5:8) {
#   barchart <- barchart + theme(axis.text.y = element_text(size = 6))  
# }
...
```{r fig4, fig.width = 6.4, fig.height = 1.7, fig.align = 'center'}
aplusb <- plot_grid(piechart, NULL, venn_obj, rel_heights = c(1,1,1), 
                    rel_widths = c(0.65, 0.05, 0.3),
                    nrow = 1, 
                    labels = c("A", "B"), 
                    label_x = -0.01, label_y = 0.9, label_size = 10) +
  theme(plot.background = element_rect(fill = "white", color = "white"))

print(aplusb)

# separate into two figures: Figure 4 
ggsave(plot = aplusb,
       units = "mm",
       dpi = 300,
       width = 140, height = 36,
       device = "png",
       filename = "./figures/longchainc_panactive_pie&venn.png")
...
ggsave(plot = aplusb,
       ...
       width = 140, height = 36,
       device = "tiff",
       filename = "./figures/300dpi/longchainc_panactive_pie&venn.tiff")
```

```{r fig5, fig.width = 8.5, fig.height = 11}
cplusd <- plot_grid(barchart, dotplot, rel_heights = c(0.6, 0.4), 
                    nrow = 2, 
                    axis = "l",
                    labels = c("A", "B"), 
                    label_x = 0, label_y = 1, label_size = 15)
...
ggsave(plot = cplusd,
       ...
       device = "png",
       filename = "./figures/longchainc_panactive_class&sel.png")
...
ggsave(plot = cplusd,
       ...
       width = 190, height = 240,
       device = "tiff",
       filename = "./figures/300dpi/longchainc_panactive_class&sel.tiff")


# combining them into one pg makes text in C&D small
# fig <- plot_grid(aplusb, cplusd,
#           rel_heights = c(0.15, 0.85), rel_widths = c(1,1), nrow = 2)
# 
# fig
# 
# ggsave(plot = fig,
...
#        width = 190, height = 240,
#        device = "tiff",
#        filename = "./figures/300dpi/longchainc_panactive_v2.tiff")
# 
# ggsave(plot = fig, 
#        units = "mm", 
#        dpi = 300, 
#        width = 190, height = 240, 
#        device = "png", 
#        filename = "./figures/longchainc_panactive_v2.png")
