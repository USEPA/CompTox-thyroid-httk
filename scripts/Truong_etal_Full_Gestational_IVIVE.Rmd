...
for (i in 1:nrow(ivive.moe.tb)) {
  ivive.moe.tb$half.life[i] <- calc_half_life(dtxsid = ivive.moe.tb$dtxsid[i], # t1/2 in hours 
                                              physchem.exclude = FALSE, 
                                              Caco2.options = list(keepit100 = TRUE)) # assume 100% bioavailability 
...
```{r add-ivive-tb, eval = execute.vignette}
...
```{r preg-aeds-RData, eval = execute.vignette}
e <- new.env(parent = emptyenv())
load('./data/invitrodb_v3_5_deiod_filtered_httk.RData', envir = e)
...
```{r mat-fet-bers-Rdata, eval = execute.vignette}
...
```{r 3compss-vs-preg-bers-Rdata, eval = execute.vignette}
