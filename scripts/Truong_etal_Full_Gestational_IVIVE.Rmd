...
## Examine Chemicals
Only 52 chemicals have experimental toxicokinetic (TK) parameters sufficient for *httk*. 
...
human.httk.dt <- subset(ac50.dt, dsstox_substance_id %in% get_cheminfo(info = 'DTXSID', 
                                                                       ...
                                                                       model = '3compartmentss', 
                                                                       median.only = TRUE))
# a lot of PFAS dropped out (n = 12)
...
1. compute a steady-state plasma concentration via `calc_mc_css` and then AED := AC50/CSS50.  
...
### Custom function to run whole pregnancy (needs to be woven into *httk*)
```{r preg-fn}
