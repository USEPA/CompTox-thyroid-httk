...
## HTTK Version 
This vignette was created with httk v2.6.0 (currently in development). 
...
```{r default-chems, eval = execute.chunk}
...
plasma.tissue.bers[, if(.N > 1) .SD, by = dtxsid]

# hide double label of 2,2'-Methylenebis(ethyl-6-tert-butylphenol)
# this occurred because min AED for this chemical was achieved in both maternal/fetal thyr
plasma.tissue.bers[which(plasma.tissue.bers$chnm == "2,2'-Methylenebis(ethyl-6-tert-butylphenol)")[2], 
                   chnm := NA_character_]

max_ber <- max(plasma.tissue.bers[, c('plasma_BER50', 'min_BER')], na.rm = T)
min_ber <- min(plasma.tissue.bers[, c('plasma_BER50', 'min_BER')], na.rm = T)
...
preg.vs.nonpreg.pp <- ggplot(data = plasma.tissue.bers, aes(x = plasma_BER50, y = min_BER)) +
  ...
  geom_label_repel(aes(label = chnm),
                   data = plasma.tissue.bers[abs(plasma_BER50 - min_BER) > 1 | plasma_BER50 < 0][
                     ! chnm %in% c("Farglitazar", "2-Chloro-N-phenylacetamide", "Fenoxaprop-P-ethyl", "Chlorhexidine diacetate") # ignore these since they're distracting
                   ],
                   size = 5, 
                   force = 65) +
  my_theme +
...
seem3pp <- ggplot(data = plasma.tissue.bers[-2, ], 
